...
run(){ echo "[$(date '+%F %T')] $1" >> "$LOG"; $PY "engine/$1" >> "$LOG" 2>&1 || echo "[$(date '+%F %T')] WARN $1 exit $?" >> "$LOG"; }
...
run a2_drawdown_rules.py        # L5 Crash/Attack Lockout (current drawdown 기준)
...
run a2_tq_dh.py                 # L5/E TQ-DH dip signals (real Reload Vault)
run a2_book.py                  # L3/L7 accountable NAV + 실제 Vault 차감
