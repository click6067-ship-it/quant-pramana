...
OUT=os.path.join(ROOT,"PRAMANA_V2"); os.makedirs(OUT,exist_ok=True)
...
# ════════════════════════════════════════════════════════════════════
d=newdoc()
d.add_heading("PRAMANA V1 — 진단 사후분석 보고서",0)
...
P(d,"명명: 여기서 'V1' = 지금까지 만든 전부(연구·검증 엔진 + Phase1A 실증 + v3 페이퍼 북). 다음 재설계는 'V2'(core-satellite, 내부 논의명 v4).",i=True,c=GRAY)
...
RED_(d,"8 레이어 자체는 건전. 문제는 이 구조가 'SPY/QQQ 코어를 충분히 들고 가는 공격형'이 아니라 '시장중립/추세/LETF를 위험예산으로 섞는 연구형'이라는 것 — core beta가 빠져 강세장에서 SPY에 구조적으로 진다. V2는 core-satellite로 교정.")
...
RED_(d,"단순팩터 사망은 '잣대가 엄해서'가 아니라 IC≈0·비용후 음수 = 진짜 엣지 없음. 여기엔 교정 불필요(정직한 negative). 단 quality는 long-only가 cap-weight 못 이긴다는 *기관식 잣대*로 FAIL 줬다 — 소액·절대수익 관점이면 '코어 대비 한계 무상관 기여' 잣대로 재심사할 가치 있음(V2 위성 후보). 단 최근 절반 decay(IC-IR 0.42→0.046)는 실재 우려.")
...
RED_(d,"NO-ML 결정은 옳다(교정 불필요). 단 DL을 *완전* 배제한 건 과했음 — alpha 예측엔 reject지만 스캐너/필터/메타라벨러(이 신호가 지금 레짐서 먹힐 확률)로는 정당. V2서 보조엔진으로 재배치.")
...
RED_(d,"② 이진 폐기의 경직성 — 통과/폐기뿐. 'standalone 엣지는 없지만 앙상블/특정 레짐선 쓸모'를 못 살림. V2: live/paper/변형/sandbox/영구폐기 + capital-veto vs research-veto 분리.")
RED_(d,"③ core 부재 — SPY/QQQ 코어 없이 위성만 섞어 강세장에서 SPY에 구조적으로 짐. core-satellite였어야.")
RED_(d,"④ analysis-paralysis — 문서 15개·게이트 14·registry 60필드인데 한동안 코드/데이터 0. 'wrong scale이 모델 아닌 프로세스에 발생'. V2: 크루드한 것부터 ship.")
...
d.save(os.path.join(OUT,"PRAMANA_V1_PostMortem.docx"))
print("✅ 1/3 PRAMANA_V1_PostMortem.docx")

# ════════════════════════════════════════════════════════════════════
# DOC 2 — V2 설계도
# ════════════════════════════════════════════════════════════════════
d=newdoc()
d.add_heading("PRAMANA V2 — 설계도",0)
P(d,"Core-Satellite + Meta-Allocator · 2026-06-11 · PAPER first (내부 논의명 v4)",i=True,c=GRAY)
...
d.save(os.path.join(OUT,"PRAMANA_V2_Design.docx"))
print("✅ 2/3 PRAMANA_V2_Design.docx")
...
H(d,"5. V1 → V2 전환")
P(d,"V1 진단: 기계는 정확(corr 0.998)·단순팩터 no-edge 정직 입증. 결함=잣대 미스칼리브레이션(기관용)·이진 폐기·core 부재(SPY 못 이김)·analysis-paralysis. 상세 → PRAMANA_V1_PostMortem.docx.")
P(d,"V2 방향: core-satellite(SPY/QQQ 코어+공격 위성)·전략군 고정 비중 레짐자동·리스크엔진 판정언어 교체(capital/research veto 분리)·DL=필터·분봉 최강격리·attribution 필수·빅뱅금지. 상세 → PRAMANA_V2_Design.docx.")
...
TBL(d,[["분류","위치","내용"],
 ...
 ["V2 산출물","PRAMANA_V2/","이 다이제스트·V1 사후분석·V2 설계도"],
