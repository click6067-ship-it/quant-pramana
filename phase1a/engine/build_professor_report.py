...
def P(t,b=False,sz=10.5,it=False,c=None):
    p=d.add_paragraph(); segs=t.split("**")          # **bold** 마크다운 → bold run 분리
    for i,seg in enumerate(segs):
        if seg=="" and len(segs)>1: continue
        r=p.add_run(seg); r.bold=(b or i%2==1); r.italic=it; r.font.size=Pt(sz)
        if c: r.font.color.rgb=RGBColor.from_string(c)
    return p
def bullet(t,sz=10):
    p=d.add_paragraph(style="List Bullet"); segs=t.split("**")
    for i,seg in enumerate(segs):
        if seg=="" and len(segs)>1: continue
        r=p.add_run(seg); r.bold=(i%2==1); r.font.size=Pt(sz)
...
P("본 연구의 기여는 '알파를 찾았다'가 아니라 '솔로가 유료 기관급 데이터(Sharadar)까지 동원한 환경에서 알파의 부재를 *사전등록·OOS·비용후·적대적 검수로 (trial registry·kill-gate 수준에서) 강하게 실증*하고, 재사용 가능한 검증 프레임워크를 구축했다'는 데 있다. 특히 적대적 검수가 look-ahead 누수를 2회 적발해 가짜 알파를 폐기한 것(§2.1)은 본 OS가 '통과'가 아니라 '기각'으로 작동하는 false-alpha rejection machine임을 보인다. Negative result이되 방법론적 엄밀성과 재현성(clone 후 one-command 재현·데이터 해시 동결)을 갖춘 정직한 연구이며, AI(LLM)를 *예측 엔진이 아니라 적대적 검수·정성 구조화 보조*로 위치시킨 운영 모델을 제시한다. 특히, public 공개 후 **외부 독립 리뷰어가 코드를 보지 않고 README만으로 내부 적대검수(Codex)와 동일한 약점**(verifiability 슬로건의 과장·8-K survivor 통계 미검증·마켓타이밍 결론의 regime 의존성)을 지적했다 — 검증 프레임의 robustness가 reviewer-orthogonal하게 외부 교차확인된 사례다(상세: External_Review.md).",False,10.5)
...
P("",False,6)
P("부속 자료: PRAMANA_Lineage_Dossier.docx(v1~V8 계보·LOCK) · PRAMANA_All_Experiments_Ledger.md · PRAMANA_Conclusions_OneLine.md · PRAMANA_A1_Attack_Book_Final.md · Live 대시보드 https://click6067-ship-it.github.io/quant-pramana/ · 코드·데이터 repo https://github.com/click6067-ship-it/quant-pramana · 외부검증 기록 https://github.com/click6067-ship-it/quant-pramana/blob/main/External_Review.md (PAPER · no live capital).",False,8.5,it=True,c="808080")
H("부록 B. Evidence Ledger — 8세대 trial 추적 (핵심 kill-test 사전등록 · 전체 trial ledger)",1)
