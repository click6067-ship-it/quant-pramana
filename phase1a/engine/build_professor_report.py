...
H("6. 결론 (Conclusion)",1)
P("유료 기관급 PIT 데이터(Sharadar)로도 개인이 SPY/QQQ를 위험조정 기준 반복적으로 초과하는 매수형 알파는 발견되지 않았다(8세대 robust negative · 저품질 데이터 탓이 아님). 그러나 본 연구는 실패가 아니라 *증명 완료*이며, 다음을 산출하였다: ① 재사용 검증 OS, ② PIT 벤치마크(corr 0.998), ③ 위험효율 우월한 생존형 코어 V7, ④ 악재 공시 회피 필터, ⑤ 가짜 알파를 자본 투입 전에 제거하는 규율. 최종적으로 프로젝트는 위험을 정직하게 인정한 공격형 페이퍼 구조 **PRAMANA A1 — Catalyst Confirmed Attack Book**(레버 ETF 없이 이벤트·정성 catalyst·비대칭 베팅으로 QQQ 초과를 *시도*; 검증된 알파가 아닌 위험예산 기반 베팅)으로 재정의된다.",False,10.5)
...
P("본 연구의 기여는 '알파를 찾았다'가 아니라 '솔로가 유료 기관급 데이터(Sharadar)까지 동원한 환경에서 알파의 부재를 *엄격한 사전등록·OOS·비용후·적대적 검수로 robust하게 실증*하고, 재사용 가능한 검증 프레임워크를 구축했다'는 데 있다. Negative result이되 방법론적 엄밀성과 재현성(clone 후 2-command 재현·데이터 해시 동결)을 갖춘 정직한 연구이며, AI(LLM)를 *예측 엔진이 아니라 적대적 검수·정성 구조화 보조*로 위치시킨 운영 모델을 제시한다.",False,10.5)
