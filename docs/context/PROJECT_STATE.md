...
**2026-06-12 V8(Levered 4-sleeve) 시도 → 기각**(용하 니즈 '상승 다 먹고 하락 방어'를 레버로): grid 1.0/1.25/1.35/1.5x crash-pack. Codex가 equity proxy 결함(VFINX≠SPY/QQQ·QQQ 닷컴 −78% 누락) 잡음 → 실 SPY/QQQ 재실행: 레버 전부 2000 닷컴 −49~−57% = V8 전면 기각·**V7 1.0x 유지.** ★정직: **V7 1.0x도 2000 닷컴 −39%**(멀티앵커 −18%는 2019~ benign·진짜 닷컴류엔 V7도 −39%·낙폭방어는 *상대적*). 알파 없이 레버로 프론티어 못 밀어냄 재실증. 옵션 컨벡시티 보류. → **관찰 모드 복귀**(V7 forward·Alpha Lab 로그). 리포트 reports/V8_Candidate_crashpack_result.md.
